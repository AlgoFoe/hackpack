...
export function createWelcomePageHTML(useTailwind) {
  ...
  <div class="z-10 max-w-6xl w-full text-center">
    ...
    <Button
      class="bg-[#dd0031] hover:bg-red-600 transition-all text-white font-semibold px-6 py-3 rounded-lg shadow-lg hover:shadow-xl"
...
export async function updateAppComponent(projectPath) {
  const filePath = path.join(projectPath, "src/app/app.ts");
  let content = await fs.readFile(filePath, "utf-8");

  // Add new imports after existing Angular core/router imports
  content = content.replace(
    /import { Component } from '@angular\/core';\s*import { RouterOutlet } from '@angular\/router';/,
    `import { Component } from '@angular/core';
import { toast, NgxSonnerToaster } from 'ngx-sonner';`
  );

  // Replace the @Component decorator to include standalone, correct styleUrls, and new imports
  content = content.replace(
    /@Component\(\{\s*selector: 'app-root',\s*imports: \[RouterOutlet\],\s*templateUrl: '.\/app.html',\s*styleUrl: '.\/app.css'\s*\}\)/,
    `@Component({
  selector: 'app-root',
  standalone: true,
  ...
  // Add the toast property inside the class - if not already present
  const toastPropertyRegex = /protected\s+readonly\s+toast\s*=\s*toast\s*;/;
  if (!toastPropertyRegex.test(content)) {
    // Insert toast property inside class App
    content = content.replace(
  ...
  console.log(
    chalk.green("\napp.component.ts updated with ngx-sonner toast support!")
  );
}
